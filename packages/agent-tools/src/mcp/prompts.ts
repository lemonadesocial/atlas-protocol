...
export function registerAtlasMcpPrompts(
  server: McpServer,
  ...
  server.registerPrompt(
    'find_events_near_me',
    {
      ...
      description:
        'Search for events across all connected platforms via Atlas Protocol',
      argsSchema: {
        city: z.string().optional().describe('City or area to search in'),
        interest: z
          .string()
          .optional()
          .describe('Type of event or topic of interest'),
      },
    },
    async ({ city, interest }) => ({
      messages: [
  ...
  server.registerPrompt(
    'compare_ticket_prices',
    {
      ...
      description:
        'Side-by-side comparison of ticket options across multiple events',
      argsSchema: {
        event_names: z
          .string()
          .optional()
  ...
  server.registerPrompt(
    'buy_tickets_for_event',
    {
      ...
      description:
        'Walk through purchasing tickets for a specific event via Atlas Protocol',
      argsSchema: {
        event_name: z
          .string()
          .optional()
          .describe('Name of the event to buy tickets for'),
      },
    },
    async ({ event_name }) => ({
      messages: [
