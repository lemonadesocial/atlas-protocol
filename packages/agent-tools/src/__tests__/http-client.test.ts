...
describe('http-client', () => {
  ...
  describe('request', () => {
    it('throws when target URL is empty', async () => {
      ...
      await expect(
        client.request({ path: '/test', target: 'registry' }),
...
describe('atlas tool flows', () => {
  ...
  describe('atlas_purchase (paid event, 402 challenge)', () => {
    it('returns 402 challenge then generates checkout URL', async () => {
      ...
      expect((purchaseResponse.data as Record<string, unknown>)['hold_id']).toBe(
        'hold_paid_456',
      );
      ...
      expect(
        String((checkoutResponse.data as Record<string, unknown>)['checkout_url']),
      ).toContain('checkout.example.test');
  ...
  describe('atlas_get_receipt', () => {
    ...
    it('returns completed status with receipt details', async () => {
      ...
      expect(
        (receiptData['reward_info'] as Record<string, unknown>)['cashback_earned'],
      ).toBe('0.10');
    });
  });
