...
The original four-stage model (below) describes the *trust* migration. Phase 5 introduces additional protocol-level milestones that operate alongside (not on top of) the trust stages — federation in Phase 5, the cross-platform transfer protocol in Phase 5.5, the marketplace and donations features in Phase 6, and on-chain `RegistryPointer` (decentralized root) in Phase 7+. Section 11 covers this Phase sequencing.
...
## 12. Phase Sequencing (Phase 5 onward)

The trust-stage model in Section 2 describes *what becomes verifiable on-chain*. Starting from Phase 5, ATLAS layers an additional sequence of protocol features on top of the trust stages. Each Phase advances horizontally — adding new functionality — while the Stages continue to advance vertically toward decentralization.

| Phase | Scope | Key deliverables | Spec references |
|-------|-------|------------------|-----------------|
| **5** | Registry federation | Self-registration API, domain-ownership verification, peer sync, multi-registry deployability via Docker, per-platform signing keys, cryptographic event provenance | [13-FEDERATION-SPEC](./13-FEDERATION-SPEC.md), [01-PROTOCOL-SPEC §1.6](./01-PROTOCOL-SPEC.md#16-registry-self-registration) |
| **5.5** | Cross-platform transfer protocol | URN supersession, signed transfer intents, federation propagation of supersession, sales-history immutability on the original platform | [14-EVENT-PORTABILITY-SPEC](./14-EVENT-PORTABILITY-SPEC.md), [02-SCHEMAS §2](./02-SCHEMAS.md#2-atlasevent) (`atlas:successor_urn`) |
| **6** | Secondary market, donations, sponsorship | On-chain ticket marketplace using AtlasTicket transferability + royalty rules; donation flows; sponsorship/match-funding primitives | TBD in implementation |
| **6.5** | Reserved | Carryover features from Phase 6 that need a follow-up cycle | TBD in implementation |
| **7+** | On-chain `RegistryPointer` (decentralized root) | The registry layer becomes optional: agents resolve events via on-chain `RegistryPointer.sol` + IPFS without contacting any registry. Multi-chain RewardLedger accrual lands in this Phase too — until then, **Base + USDC is the canonical reward chain**. | [04-SMART-CONTRACTS-SPEC §6](./04-SMART-CONTRACTS-SPEC.md#6-registrypointersol), [04-SMART-CONTRACTS-SPEC §5](./04-SMART-CONTRACTS-SPEC.md#5-rewardledgersol) |

**RewardLedger canonicality.** Until Phase 7+, `RewardLedger` v1 lives on **Base + USDC** only. Settlement on other chains routes the reward portion of the protocol fee to Base for accrual. This bounds the timelock-and-claim UX to a single chain in v1 and avoids the cross-chain reward-bridging complexity of a multi-chain ledger. Phase 7+ introduces multi-chain RewardLedger deployments and the canonical-chain bridge becomes a governance choice rather than a protocol constant.

**Phase ordering vs Stage ordering.** A Phase 5 federation deployment can sit at Stage 1 (FeeRouter on-chain, ticket NFTs not yet) or at Stage 4 (RegistryPointer on-chain). The two axes are orthogonal. Implementations track them as separate dimensions in their deployment metadata.
