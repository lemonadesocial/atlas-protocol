...
### 1.1 Multi-Registry Configuration

Phase 5 introduces federated registries (see [13-FEDERATION-SPEC](./13-FEDERATION-SPEC.md)). Agents MAY query a single registry (the default) or aggregate results from multiple registries. The SDK accepts a `registries: string[]` option:

```typescript
const atlas = new ATLAS({
  registries: [
    'https://registry.atlas.events',           // primary
    'https://registry.latam.atlas.events',     // regional partner
    'https://my-self-hosted-registry.example'  // operator's own node
  ]
})
```

When multiple registries are configured, the SDK fans out searches in parallel and merges results de-duplicated by `atlas:urn`. If two registries return conflicting metadata for the same URN, the SDK MUST surface the conflict to the agent (do not silently pick one). Registry trust scoring is the agent's responsibility — the protocol does not designate a canonical registry.

Default discovery uses DNS SRV records (`_atlas-registry._tcp.atlas.events`) to locate registries; agents that override `registries` skip discovery.

### 1.2 ATLAS-Managed Mint and Settle Endpoints

In Phase 5, AtlasTicket holds `MINTER_ROLE` and `BURNER_ROLE` exclusively in the ATLAS protocol multisig (see [04-SMART-CONTRACTS-SPEC §4](./04-SMART-CONTRACTS-SPEC.md#4-atlasticketsol-erc-721)). Agents do not call mint or burn directly. Instead they invoke ATLAS-managed endpoints:

| Endpoint | Purpose |
|----------|---------|
| `POST /atlas/v1/mint` | Request mint of a ticket against a settled hold. ATLAS validates the FeeRouter settlement, enforces resale rules, and submits the on-chain mint via meta-transaction (gas subsidized by the 0.5% protocol fee). |
| `POST /atlas/v1/settle` | Request settlement when the agent does not hold the buyer's gas. ATLAS relays the EIP-712 signed payment intent. |
| `POST /atlas/v1/burn` | Request burn (refund/loss/replacement). ATLAS validates the originating reason and submits the burn. |

This concentrates `MINTER_ROLE` enforcement at the protocol layer — no per-platform key sprawl, no per-platform proof-of-fee-compliance audit. Agents authenticate to these endpoints with their `Atlas-Agent-Id` and `atlas_sk_live_*` API key.
...
**Output:** A receipt credential (W3C Verifiable Credential) containing the event URN, ticket type, quantity, settlement chain, and IPFS CID of the **proof-of-issuance** (NOT the full receipt — see [05-IPFS-DATA-LAYER §6](./05-IPFS-DATA-LAYER.md#6-receipt-proof-of-issuance)). The full receipt is delivered to the buyer's wallet; only the issuance proof is on IPFS.

### 2.5 atlas_list_my_tickets (Wallet Aggregator)

Lists all ATLAS tickets held by a buyer across every chain AtlasTicket is deployed on. The buyer specifies the wallet (or email-keyed custodial address) and the agent receives a unified ticket portfolio without having to query each chain individually.

**Input parameters:**

| Param | Type | Required | Description |
|-------|------|----------|-------------|
| holder | string | yes | Wallet address or `did:web` of the buyer. Email lookups MUST be hashed by the caller. |
| chains | string[] | no | Optional chain filter. Defaults to all chains AtlasTicket is deployed on. |
| include_superseded | boolean | no | If true, include tickets whose event has been superseded via the transfer protocol (see [14-EVENT-PORTABILITY-SPEC](./14-EVENT-PORTABILITY-SPEC.md)). Default `false`. |

**Output:** Array of `{token_id, chain, event_urn, ticket_type, holder, current_event_urn}`. `current_event_urn` differs from `event_urn` when the original event has been transferred; the buyer's ticket remains valid on the original platform's sales record but the canonical event has moved. The wallet aggregator is operated by the ATLAS-managed endpoint `GET /atlas/v1/wallet/<holder>/tickets`; the SDK abstracts the chain fan-out.
