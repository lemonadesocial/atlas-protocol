...
The manifest schema is defined in [02-SCHEMAS §1 AtlasManifest](./02-SCHEMAS.md#1-atlasmanifest) — that document is the canonical source of truth. The shape below reproduces it for convenience.
...
- `url`: Platform homepage URL.
- `logo`: Optional URL to a platform logo (PNG or SVG, min 128x128).
- `capabilities`: Array of supported protocol features. Valid values: `listing`, `purchase`, `settlement`.
- `endpoints`: Object containing API endpoint URLs. MUST include `events_url` (event feed) and `purchase_url` (402 flow base).
- `settlement.chains`: Array of accepted settlement-chain identifiers (snake_case bare names: e.g. `base`, `arbitrum`, `worldchain`, `megaeth`, `tempo`). Settlement-token suffixes are NOT used here — the token is reported separately in `settlement.token`.
- `settlement.token`: Settlement token symbol, e.g. `USDC`.
